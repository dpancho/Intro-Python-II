...
while True:
    ...
    cmd = input(
        "What would you like to do? M to Move, T to Take, or Q to Quit the game. ~~~>").lower().strip()
    if cmd == 'q':
        break
    elif cmd == 'm':
        move = input(
        ...
        item = input(
            f"What do you want to take or Q to Quit? {player.current_room.item} ~~~> ").lower().strip()
        if cmd == 'q':
            break
